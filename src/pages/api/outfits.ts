...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (isDemoRequest(req)) {
      ...
      switch (req.method) {
        ...
        case "POST": {
          ...
          if (!userId || !name || !items) return res.status(400).json({ error: "userId, name, and items are required" });
    ...
    switch (req.method) {
      case "GET": {
        const { userId, id } = req.query;
        if (id) { const outfit = await outfitRepo.getById(id as string); if (!outfit) return res.status(404).json({ error: "Outfit not found" }); return res.status(200).json(outfit); }
        if (userId) { const outfits = await outfitRepo.getByUserId(userId as string); return res.status(200).json(outfits); }
        const outfits = await outfitRepo.getPublic();
        return res.status(200).json(outfits);
      }
      case "POST": {
